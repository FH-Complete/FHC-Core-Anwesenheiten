	...
	methods: {
		showLvaAnw(quota){
			if(!quota.showDetails && !quota.detailsLoaded) {
				quota.currentlyLoadingDetails = true;
				this.$api.call(ApiProfil.getAnwesenheitenByLva(null, quota.lehrveranstaltung_id, quota.prestudent_id, this.uid))
					.then(res => {
						...
						quota.details.forEach(d => {
							d.vonDate = this.formatShortDate(new Date(d.von))
							d.vonTime = this.formatTime(new Date(d.von)) 
							d.bisTime = this.formatTime(new Date(d.bis))
						})
					}).finally(()=>{
					quota.currentlyLoadingDetails = false;
				})
